	...
	socket.on('createCheckoutSession', async (data) => {
		try {
			...
			const session = await stripe.checkout.sessions.create({
  				line_items,
  				mode: 'payment',
  				success_url: '/', // replace with url that checkout screen should pop up upon success
  				cancel_url: '/', // replace with the url that checkout screen should pop up upon cancelling
			});
